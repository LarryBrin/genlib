    ...
    for language in LANGUAGES:
        ...
        for item in results:
            ...
            # for i in item:
                ...
                # f. write(str(i)+'\n')
    python_list = []
    for item in info['python'].values():
        python_list.append((item['title'], item['year'],))
    # print(python_tuple)
    # len_list = len(python_tuple)
    # len_set = len(set(python_tuple))
    # print(len_set, '+', len_list)
    len_list = len(python_list)
    print('len_list: ',len_list)
    python_set = set(python_list)
    len_set = len(python_set)
    print('len_set: ', len_set)
    # print(python_set)
    python_list.sort(key=lambda x: x[0])
    # print(python_list)
    new_list = list(python_set)
    new_list.sort(key=lambda x: x[0])
    # print(new_list)
    processed_list = []
    for item in new_list:
        if 'python' not in item[0]:
            continue
        elif len(item[1]) < 4:
            continue
        if ':' in item[0]:
